...
import { collectionRoutes, guideHubRoutes, previewRoutes, seoPages } from './seoLibrary'

export function pageForPath(pathname: string) {
  const normalized = pathname.replace(/\/$/, '') || '/'
  const basePage = [...pages, ...betaPages, ...previewRoutes, ...guideHubRoutes].find((page) => page.path === normalized)
